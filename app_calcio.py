...
# --- CSS INTELLIGENTE (COMPATIBILE CON DARK & LIGHT MODE) ---
st.markdown("""
    <style>
    /* Nasconde le catenine vicino ai titoli */
    .element-container h1 a, .element-container h2 a, .element-container h3 a { display: none; }
    
    /* Titoli che si adattano al tema */
    h1, h2, h3 { margin-top: -10px; padding-bottom: 10px; }
    
    /* Riquadri delle metriche (Card) con trasparenza per adattarsi allo sfondo */
    ...
    /* Forza il colore del testo nelle metriche per essere leggibile */
...
# --- CALCOLO MEDIE PESATE (60% Forma - 40% Stagione) ---
...
with col_res:
    ...
    # Formattazione Giallo Excel (Sempre nero su giallo per massima leggibilità)
...
scenarios_unique = list(dict.fromkeys(top_scenarios_list)) # Rimuove duplicati mantenendo l'ordine
c_sce = st.columns(len(scenarios_unique))
