...
with tab1:
    ...
    with c_c2:
        ...
        def hl(r): return ['background-color: #ffff00; color: black; font-weight: bold']*3 if r['Risultato'] in scen else ['']*3
    ...
    for i, r_n in enumerate(scen):
        ...
        with cols_s[i]:
            st.metric(label="ESATTO", value=r_n, delta=f"{p_val:.1f}% (QF: {100/p_val:.2f})")
            if st.button(f"📌 Invia {r_n}", key=f"bt_res_{i}"): salva_in_db(f"Esatto {r_n}")
    ...
    cc = st.columns(3)
    p_bil = get_combo_p(rc[0], rc[1], ro[0], ro[1])
    n_bil = f"CASA {rc[0]}-{rc[1]} + OSPITE {ro[0]}-{ro[1]}"
    with cc[0]:
        st.metric("BILANCIATO", n_bil, delta=f"{p_bil:.1f}% (QF: {100/p_bil:.2f})")
        if st.button("📌 Invia Bil", key="btn_bil"): salva_in_db(f"Bil: {n_bil}")
    
    if exp_c >= exp_o:
        lab_d, n_d, p_d = "DOMINIO CASA", f"CASA {rc[0]}-{rc[1]} + OSPITE 0-1", get_combo_p(rc[0], rc[1], 0, 1)
    else:
        lab_d, n_d, p_d = "DOMINIO OSPITE", f"CASA 0-1 + OSPITE {ro[0]}-{ro[1]}", get_combo_p(0, 1, ro[0], ro[1])
    with cc[1]:
        st.metric(lab_d, n_d, delta=f"{p_d:.1f}% (QF: {100/p_d:.2f})")
        if st.button(f"📌 Invia {lab_d}", key="btn_dom"): salva_in_db(f"Dom: {n_d}")
        
    p_go = get_combo_p(1,3,1,3)
    with cc[2]:
        st.metric("COMBO GOAL", "CASA 1-3 + OSPITE 1-3", delta=f"{p_go:.1f}% (QF: {100/p_go:.2f})")
        if st.button("📌 Invia Goal", key="btn_goal"): salva_in_db("Combo Goal: CASA 1-3 + OSPITE 1-3")
    ...
    m_cols = st.columns(6)
    m_cols[0].metric("1", f"{p1:.1f}%", f"QF:{100/p1:.2f}")
    m_cols[1].metric("X", f"{px:.1f}%", f"QF:{100/px:.2f}")
    m_cols[2].metric("2", f"{p2:.1f}%", f"QF:{100/p2:.2f}")
    m_cols[3].metric("Over 2.5", f"{ov25:.1f}%", f"QF:{100/ov25:.2f}")
    m_cols[4].metric("GOAL", f"{pg:.1f}%", f"QF:{100/pg:.2f}")
    m_cols[5].metric("NO GOAL", f"{100-pg:.1f}%", f"QF:{100/(100-pg):.2f}")
    ...
    cmg = st.columns(4)
    for i, mg in enumerate(mg_l): cmg[i % 4].metric(f"MG {mg[0]}-{mg[1]}", f"{gmm(mg[0], mg[1]):.1f}%", f"QF:{100/gmm(mg[0], mg[1]):.2f}")
