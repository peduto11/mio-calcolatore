...
if not is_hockey:
    ...
    max_g = 6 # Calcio fino a 5
else:
    # 🏒 INPUT HOCKEY (CLASSIFICA)
...
with tab1:
    ...
    # --- RIPRISTINO SCENARI ESATTI ---
    st.subheader("💡 Scenari Esatti")
    cs = st.columns(4)
    for i, rn in enumerate(scen[:4]): # Mostra fino a 4 scenari
        try:
    ...
    # --- RIPRISTINO SCENARI COMBO (CON LOGICA ADATTIVA) ---
    ...
    if not is_hockey:
        ...
        # Range Hockey
        rc = (1,2) if ex_c < 2.0 else (2,4) if ex_c < 3.5 else (3,5)
        ro = (1,2) if ex_o < 2.0 else (2,4) if ex_o < 3.5 else (3,5)
        g_min, g_max = 2, 4 # In hockey il "Goal" è almeno 2 gol a testa
    ...
    p_go = gp(g_min, g_max, g_min, g_max)
    with cb[2]:
        n_go = f"T1 {g_min}-{g_max} + T2 {g_min}-{g_max}"
        st.metric("COMBO GOAL", n_go, delta=f"{p_go:.1f}% (QF:{100/p_go:.2f})" if p_go>0 else "0")
        if st.button("📌 Invia Combo Goal"): add_to_db(f"Combo: {n_go}")

    # --- RIPRISTINO MERCATI COMPLETI ---
    ...
    mc = st.columns(6)
    mc[0].metric("1 (Reg Time)", f"{p1:.1f}%", f"QF:{100/p1:.2f}")
    mc[1].metric("X (Reg Time)", f"{px:.1f}%", f"QF:{100/px:.2f}")
    mc[2].metric("2 (Reg Time)", f"{p2:.1f}%", f"QF:{100/p2:.2f}")
    
    if not is_hockey:
        ov, pg = over_prob(2.5), sum(matrix[h, a] for h in range(1, max_g) for a in range(1, max_g)) * 100
        mc[3].metric("O2.5", f"{ov:.1f}%", f"QF:{100/ov:.2f}")
        ...
        mc[3].metric("OVER 4.5", f"{o45:.1f}%", f"QF:{100/o45:.2f}")
        mc[4].metric("OVER 5.5", f"{o55:.1f}%", f"QF:{100/o55:.2f}")
        mc[5].metric("UNDER 5.5", f"{(100-o55):.1f}%", f"QF:{100/(100-o55):.2f}")
        mg_list = [(3,5), (3,6), (4,6), (4,7), (5,7), (5,8), (6,8), (6,9)]
    ...
    # --- RIPRISTINO BLOCCO INFERIORE (MG CASA / MG OSPITE / DC) ---
    st.markdown("---")
    cd1, cd2, cd3 = st.columns(3)
    
    if not is_hockey: mg_team_list = [(1,2), (1,3), (2,3)]
    else: mg_team_list = [(2,3), (2,4), (3,4)]
    
    with cd1:
        st.write(f"**🏠 MG {t_h[:8].upper()}**")
        for l, h in mg_team_list:
