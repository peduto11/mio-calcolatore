...
st.markdown("""
    ...
    /* Stile per tutte le schede (metriche) */
...
m_cols[5].metric("NO GOAL", f"{p_ng:.1f}%", f"QF:{100/p_ng:.2f}")
...
# --- MG CASA, OSPITE E DOPPIA CHANCE (CON SCHEDE) ---
st.markdown("---")
...
with c_down1:
    ...
    for l, h in [(1,2), (1,3), (2,3)]:
        p_c = sum(prob_c_list[i] for i in range(l, h+1)) * 100
        st.metric(f"Casa {l}-{h}", f"{p_c:.1f}%", f"QF: {100/p_c:.2f}")

with c_down2:
    st.write("**🚀 MULTIGOL OSPITE**")
    prob_o_list = [poisson(exp_o, i) for i in range(max_g)]
    for l, h in [(1,2), (1,3), (2,3)]:
        p_o = sum(prob_o_list[i] for i in range(l, h+1)) * 100
        st.metric(f"Ospite {l}-{h}", f"{p_o:.1f}%", f"QF: {100/p_o:.2f}")

with c_down3:
    st.write("**⚖️ DOPPIA CHANCE**")
    st.metric("1X (Casa o Pareggio)", f"{(p1+px):.1f}%", f"QF: {100/(p1+px):.2f}")
    st.metric("X2 (Pareggio o Ospite)", f"{(p2+px):.1f}%", f"QF: {100/(p2+px):.2f}")
    st.metric("12 (Vince uno o l'altro)", f"{(p1+p2):.1f}%", f"QF: {100/(p1+p2):.2f}")
