...
with tab1:
    ...
    for i, res_n in enumerate(top_scen):
        ...
        c_sce[i].metric(f"SCENARIO: {res_n}", f"{p_s:.1f}%", f"QF: {100/p_s:.2f}")
...
with tab2:
    ...
    q_b1, q_bx, q_b2 = 100/b1, 100/bx, 100/b2

    df_bvs = pd.DataFrame({
        "Segno": ["1", "X", "2"],
        "Probabilità BVS": [f"{b1:.2f}%", f"{bx:.2f}%", f"{b2:.2f}%"],
        "Quota Fair BVS": [round(q_b1, 2), round(q_bx, 2), round(q_b2, 2)],
        "Quota Bookmaker": [q1_book, qx_book, q2_book],
        "VALORE": ["✅ SI" if q1_book > q_b1 else "❌ NO", "✅ SI" if qx_book > q_bx else "❌ NO", "✅ SI" if q2_book > q_b2 else "❌ NO"]
    })
    st.table(df_bvs)
    st.info("💡 Confronta le quote: se vedi '✅ SI', il bookmaker sta offrendo una quota più alta della stima BVS.")
